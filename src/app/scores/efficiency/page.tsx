        ...
        <div className="dev-surface p-4 md:p-8">
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-center mb-8">
            <div className="w-full lg:flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="relative group">
                <TrendingUp className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-500" />
                <Input
            ...
            <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
              <Button
                ...
                className={cn(
                  "flex-1 md:flex-none h-12 bg-[#0c0c0f] border border-white/[0.06] rounded-xl text-xs font-bold uppercase tracking-wider transition-colors px-6",
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="bg-[#141418] border border-white/[0.06] text-zinc-400 min-w-[200px] rounded-xl p-2 shadow-2xl">
                  <DropdownMenuItem
                    className="flex items-center gap-3 hover:bg-white/[0.04] cursor-pointer text-xs font-bold uppercase tracking-wider p-3 rounded-lg mb-1"
                    onClick={() => setHasLinkedIn(!hasLinkedIn)}
                  >
                    <div
                      className={cn(
                        "h-5 w-5 rounded border flex items-center justify-center transition-colors",
                        hasLinkedIn ? "bg-indigo-500 border-indigo-500" : "bg-[#0c0c0f] border-white/20"
                      )}
                    >
                  ...
                  <DropdownMenuItem
                    className="flex items-center gap-3 hover:bg-white/[0.04] cursor-pointer text-xs font-bold uppercase tracking-wider p-3 rounded-lg"
                    onClick={() => setHasEmail(!hasEmail)}
                  >
                    <div
                      className={cn(
                        "h-5 w-5 rounded border flex items-center justify-center transition-colors",
                        hasEmail ? "bg-indigo-500 border-indigo-500" : "bg-[#0c0c0f] border-white/20"
                      )}
                    >
                      {hasEmail && <Check className="h-3.5 w-3.5 text-white" />}
                    </div>
                    Email
                  </DropdownMenuItem>
                </DropdownMenuContent>
                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {sortBy === "contributorEfficiency" ? "Efficiency" : sortBy === "totalScore" ? "Total score" : "Contributions"}
                    </span>
                    <ChevronDown className="h-4 w-4 opacity-50" />
                  </Button>
                </DropdownMenuTrigger>
              ...
              <Table>
                ...
                <TableBody>
                  {data?.data.length ? (
                      ...
                      <>
                          ...
                          <TableCell className="px-6 py-4">
                            <div className="flex items-center gap-4">
                              <Avatar className="h-10 w-10 border border-white/[0.06] rounded-lg">
                                ...
                                <AvatarFallback className="rounded-lg bg-[#18181c] text-zinc-400">
                              ...
                              <div className="flex flex-col text-left">
                                ...
                                <div className="flex items-center gap-2">
                                  <span className="text-xs text-zinc-500 mr-2 uppercase tracking-wide">
                                    @{entry.username}
                                  </span>
                                  {entry.email && (
                                    <a href={`mailto:${entry.email}`} onClick={(e) => e.stopPropagation()} className="text-zinc-500 hover:text-white">
                                      <Mail className="h-3 w-3" />
                                    </a>
                                  )}
                                  {entry.linkedin && (
                                    <a href={entry.linkedin.startsWith("http") ? entry.linkedin : `https://${entry.linkedin}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="text-zinc-500 hover:text-indigo-400">
                                      <Linkedin className="h-3 w-3" />
                                    </a>
                                  )}
                                  {entry.twitterUsername && (
                                    <a href={`https://twitter.com/${entry.twitterUsername}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="text-zinc-500 hover:text-indigo-400">
                        ...
                        {expanded[entry.username] && (
                          <TableRow className="bg-[#0c0c0f] border-white/[0.04] hover:bg-transparent">
                            <TableCell colSpan={7} className="p-0 border-b border-white/[0.04]">
                              <div className="p-6 md:p-10 animate-in fade-in slide-in-from-top-2 duration-300">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                  <div className="p-4 bg-[#141418] rounded-xl border border-white/[0.04]">
                                    <p className="text-xs text-zinc-500 font-bold uppercase tracking-wide mb-2">AI Score</p>
                                    <p className="text-2xl font-bold text-violet-400">{(entry.aiScore || 0).toFixed(1)}</p>
                                  </div>
                                  <div className="p-4 bg-[#141418] rounded-xl border border-white/[0.04]">
                                    <p className="text-xs text-zinc-500 font-bold uppercase tracking-wide mb-2">Backend Score</p>
                                    <p className="text-2xl font-bold text-indigo-400">{(entry.backendScore || 0).toFixed(1)}</p>
                                  </div>
                                  <div className="p-4 bg-[#141418] rounded-xl border border-white/[0.04]">
                                    <p className="text-xs text-zinc-500 font-bold uppercase tracking-wide mb-2">Frontend Score</p>
                                    <p className="text-2xl font-bold text-[#F59E0B]">{(entry.frontendScore || 0).toFixed(1)}</p>
                                  </div>
                                  <div className="p-4 bg-[#141418] rounded-xl border border-white/[0.04]">
                                    <p className="text-xs text-zinc-500 font-bold uppercase tracking-wide mb-2">DevOps Score</p>
                                    <p className="text-2xl font-bold text-[#EF4444]">{(entry.devopsScore || 0).toFixed(1)}</p>
                                  </div>
                                </div>
                                {entry.bio && (
                                  <p className="mt-6 text-sm text-zinc-500 leading-relaxed border-l-2 border-indigo-500/30 pl-4 italic">
                                    &ldquo;{entry.bio}&rdquo;
                                  </p>
                                )}
                                <div className="flex gap-4 mt-6">
                                  <Link href={`/user/${entry.username}`} onClick={(e) => e.stopPropagation()}>
                                    <Button className="bg-indigo-500 text-white hover:bg-indigo-500/90 h-10 rounded-lg font-bold uppercase tracking-wider text-xs">
                                      View profile
                                    </Button>
                                  </Link>
                                  <a href={`https://github.com/${entry.username}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}>
                                    <Button variant="outline" className="h-10 border-white/[0.06] bg-[#141418] text-white hover:bg-[#18181c] rounded-lg flex items-center gap-2 font-bold uppercase tracking-wider text-xs">
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="h-32 text-center text-zinc-500 font-bold tracking-wide uppercase text-xs">
